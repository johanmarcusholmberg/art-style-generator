...
import { WHIMSICALJAPANESE_STYLE } from "@/lib/style-config";
import StyleNav from "@/components/StyleNav";

const styleConfig = WHIMSICALJAPANESE_STYLE;

const WhimsicalJapanese = () => {
  ...
  return (
    <div className="min-h-screen bg-background paper-texture">
      <StyleNav activePath="/whimsical-japanese" />
      ...
      <header className="pt-10 pb-12 text-center px-4">
        <p className="font-display text-muted-foreground text-sm tracking-[0.3em] uppercase mb-3">
          Whimsical Japanese · Storybook Poster
        </p>
        <h1 className="font-display text-4xl sm:text-5xl md:text-6xl font-bold text-foreground leading-tight mb-4">
          Whimsical Japanese<br />
          <span className="text-primary">Poster Generator</span>
        </h1>
        <p className="text-muted-foreground max-w-lg mx-auto text-sm leading-relaxed">
          Charming anthropomorphic characters in Japanese-inspired scenes — soft gouache,
          hand-inked outlines, muted vintage palette, calm storybook mood.
        </p>
      ...
      <footer className="pb-8 text-center">
        <p className="text-muted-foreground text-xs font-display tracking-widest">🦊 Storybook Studio</p>
      </footer>
...
export default WhimsicalJapanese;
