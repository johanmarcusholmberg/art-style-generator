CREATE OR REPLACE FUNCTION public.create_matching_collection_atomic(p_fingerprint text, p_name text, p_anchor_image_id uuid, p_anchor_image_url text, p_anchor_storage_path text, p_anchor_width_px integer, p_anchor_height_px integer, p_anchor_aspect_ratio text, p_anchor_style_key text, p_anchor_poster_format_id text, p_anchor_background_style text, p_anchor_provider text, p_anchor_model text, p_resolved_provider text, p_resolved_model text, p_provider_preference text, p_provider_substitution_reason text, p_art_direction jsonb, p_art_direction_version integer, p_consistency_strength text, p_reference_strength text, p_job_idempotency_key text, p_job_prompt text, p_items jsonb)
...
BEGIN
  ...
  -- Anchor validation for the single-admin data model: generated_images has
  -- no per-user ownership column, so authorization is admin-scoped and the
  -- anchor is validated by existence + not-deleted.
  IF NOT public.is_current_user_admin() THEN
    RAISE EXCEPTION 'forbidden';
  END IF;

  IF p_anchor_image_id IS NOT NULL
     AND NOT EXISTS (
       SELECT 1
  ...
  -- Reject cross-collection idempotency-key reuse.
  ...
  -- Orphan fingerprint: collection row exists but no job attached yet.
  ...
  -- Normalize items: strip caller-provided matchingCollectionId and inject
  -- the authoritative collection id.
  SELECT jsonb_agg(
...
GRANT EXECUTE ON FUNCTION public.create_matching_collection_atomic(text, text, uuid, text, text, integer, integer, text, text, text, text, text, text, text, text, text, text, jsonb, integer, text, text, text, text, jsonb) TO authenticated;