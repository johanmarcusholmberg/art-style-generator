...
BEGIN
  ...
  IF p_mode = 'archive_root' THEN
    UPDATE public.generated_images
       SET admin_status = 'archived', is_archived = true, updated_at = now()
     WHERE id = p_root_image_id;
  ...
  IF p_mode = 'archive_asset' THEN
    ...
    RETURN jsonb_build_object('ok', true, 'mode', p_mode, 'noop', v_noop OR NOT FOUND,
      'promoted_asset_id', p_promote_asset_id,
      'deleted_asset_ids', CASE WHEN v_noop THEN '[]'::jsonb ELSE to_jsonb(ARRAY[p_asset_id]) END,
      'storage_paths_safe_to_remove', ARRAY[]::text[]);
  END IF;
