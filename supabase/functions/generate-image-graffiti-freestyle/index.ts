...
serve(async (req) => {
  ...
  try {
    ...
    if (sourceImageUrl) {
      const editPrompt = `CRITICAL: You MUST keep the provided image almost entirely unchanged. Only make the SPECIFIC edit described below — preserve the exact same composition, subjects, colors, background, perspective, and every other detail. The result must look like the same image with a small targeted modification, NOT a new image. Do NOT regenerate or reimagine the scene. Keep the graffiti/street art style.${bgText} Do NOT include any text or written script in the image. Only apply the art style, nothing else. Specific edit to apply: ${trimmedPrompt}. Generate at maximum resolution.${ratioText}${frameText}`;
      messages = [{ role: "user", content: [{ type: "image_url", image_url: { url: sourceImageUrl } }, { type: "text", text: editPrompt }] }];
    } else {
      const enhancedPrompt = `Create a high-resolution, highly detailed image. Render the following scene in a graffiti/urban street art style — use vibrant spray paint colors, bold outlines, dripping paint effects, stencil art elements, urban grit and texture, inspired by Banksy, KAWS, and classic street art.${bgText} The subject itself should be exactly as described, rendered in street art style. Do NOT include any text or written script in the image. Only apply the art style, nothing else. Generate at maximum resolution with crisp detail suitable for large format printing: ${trimmedPrompt}.${ratioText}${frameText}`;
    ...
    console.error("generate-image-graffiti-freestyle error:", e);
