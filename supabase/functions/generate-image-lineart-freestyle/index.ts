...
const STYLE_RULES = {
  style: ["fine pen-and-ink line art style", "delicate ink lines with hatching for depth", "elegant pen technique with varying weights"],
  ...
  quality: ["consistent crisp linework", "fine detail throughout", "high detail", "professional illustration", "sharp edges", "no artifacts", "print-ready resolution"],
...
serve(async (req) => {
  ...
  } catch (e) { console.error("generate-image-lineart-freestyle error:", e); return new Response(JSON.stringify({ error: "An unexpected error occurred." }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } }); }
});
