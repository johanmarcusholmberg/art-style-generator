...
function buildPrompt(p: string, ar?: string, bg?: string): string {
  const bgText = bg === "cream" ? "Use a traditional warm beige/cream washi paper texture as the background." : "The background MUST be pure white (#FFFFFF). Do NOT use cream, beige, off-white, or any tinted paper color.";
  ...
  return [
    ...
    "",
    `STYLE RULES: ${RULES.style.join(". ")}`,
    "",
    `COMPOSITION: ${RULES.composition.join(". ")}`,
    "",
    `COLOR: ${RULES.color.join(". ")}`,
    "",
    `GLOBAL QUALITY: ${RULES.quality.join(". ")}`,
    "",
...
function buildEditPrompt(p: string, ar?: string, bg?: string): string {
  ...
  return [
    ...
    "Preserve the exact same composition, subjects, colors, background, perspective, lighting.",
    "Do NOT regenerate or reimagine the scene.",
    "",
    `VISUAL GOAL: ${RULES.visualGoal.join(". ")}`,
    `STYLE ANCHORS: ${RULES.styleAnchors.join(", ")}`,
    `STYLE TO MAINTAIN: ${RULES.style.join(", ")}`,
    "",
    `EDIT TO APPLY: ${p}`,
    "",
    bgText,
    ar ? `Maintain ${ar} aspect ratio.` : "",
    `GLOBAL QUALITY: ${RULES.quality.join(", ")}`,
    `AVOID: ${RULES.avoid.join(", ")}`,
