...
serve(async (req) => {
  ...
  try {
    ...
    if (sourceImageUrl) {
      const editPrompt = `CRITICAL: You MUST keep the provided image almost entirely unchanged. Only make the SPECIFIC edit described below — preserve the exact same composition, subjects, colors, background, perspective, and every other detail. The result must look like the same image with a small targeted modification, NOT a new image. Do NOT regenerate or reimagine the scene. Keep the minimalist art style — clean shapes, limited palette, generous negative space. Do NOT include any text or written script in the image. Only apply the art style, nothing else. Specific edit to apply: ${trimmedPrompt}. Generate at maximum resolution.${ratioText}${frameText}${marginText}`;
      messages = [
        {
          role: "user",
          content: [
      ...
      const enhancedPrompt = `Create a high-resolution minimalist artwork: ${trimmedPrompt}. Style: clean geometric shapes, limited color palette of 2-4 muted tones, generous negative space, modern composition, flat design with subtle depth, Swiss/Scandinavian design influence, abstract simplification of forms, precise edges, restrained and elegant. No clutter, no excessive detail — every element must be intentional. Generate at maximum resolution with crisp detail suitable for large format printing.${ratioText}${frameText}${marginText}`;
