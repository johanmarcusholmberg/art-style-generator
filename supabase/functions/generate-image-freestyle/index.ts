...
function buildPrompt(p: string, ar?: string, bg?: string): string {
  const bgText = bg === "cream" ? "Use a traditional warm beige/cream washi paper texture." : "The background MUST be pure white (#FFFFFF). Do NOT use cream, beige, off-white, or any tinted paper color.";
...
function buildEditPrompt(p: string, ar?: string, bg?: string): string {
  const bgText = bg === "cream" ? "Use a traditional warm beige/cream washi paper texture." : "The background MUST be pure white (#FFFFFF).";
  return ["CRITICAL EDITING INSTRUCTIONS:", "You MUST keep the provided image almost entirely unchanged.", "Only make the SPECIFIC edit described below.", "Preserve the exact same composition, subjects, colors, background, perspective, lighting.", "Do NOT regenerate or reimagine the scene.", "", `VISUAL GOAL: ${RULES.visualGoal.join(". ")}`, `STYLE ANCHORS: ${RULES.styleAnchors.join(", ")}`, `STYLE TO MAINTAIN: ${RULES.style.join(", ")}`, "", `EDIT TO APPLY: ${p}`, "", bgText, ar ? `Maintain ${ar} aspect ratio.` : "", `GLOBAL QUALITY: ${RULES.quality.join(", ")}`, `AVOID: ${RULES.avoid.join(", ")}`, "Generate at maximum resolution."].filter(Boolean).join("\n");
}
