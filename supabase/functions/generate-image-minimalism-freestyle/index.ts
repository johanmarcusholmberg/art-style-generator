...
const STYLE_RULES = {
  style: ["minimalist art style with clean simplified forms", "geometric shapes and flat design", "Scandinavian design aesthetic"],
  ...
  quality: ["precise clean edges", "professional quality", "high detail", "sharp edges", "no artifacts", "print-ready resolution"],
  avoid: ["visual clutter", "excessive detail", "any written text or script"],
};

function buildPrompt(p: string, ar?: string, bg?: string): string {
  const bgText = bg === "cream" ? "Use a warm cream/off-white paper background tone." : "The background MUST be pure white (#FFFFFF).";
...
serve(async (req) => {
  ...
  } catch (e) { console.error("generate-image-minimalism-freestyle error:", e); return new Response(JSON.stringify({ error: "An unexpected error occurred." }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } }); }
});
