...
const STYLE_RULES = {
  style: ["ultra-minimal continuous line drawing", "absolute fewest lines possible to convey the subject", "single-weight thin black line", "inspired by Picasso's single-line drawings", "one-line art style with elegant simplicity"],
  ...
  quality: ["perfectly smooth continuous line", "elegant confident strokes", "museum-quality minimal art", "high detail", "sharp edges", "no artifacts", "print-ready resolution"],
...
function buildPrompt(p: string, ar?: string, bg?: string): string {
  const bgText = bg === "cream" ? "The background should be a warm cream/off-white vintage paper tone." : "The background MUST be pure white (#FFFFFF).";
...
serve(async (req) => {
  ...
  } catch (e) { console.error("generate-image-lineart-minimal error:", e); return new Response(JSON.stringify({ error: "An unexpected error occurred." }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } }); }
});
